...
function render(){ if(localStorage.getItem('radar-theme')==='dark'&&!state.dark){state.dark=true;document.documentElement.classList.add('dark');} syncThemeUI(); renderChrome(); if(state.view!=='workspace'&&window.WorkspaceView){window.WorkspaceView.hide();} if(state.view==='detail'){renderDetail(new URLSearchParams(location.search).get('paper'));return;} if(state.view==='workspace'){renderHome(false);window.WorkspaceView&&window.WorkspaceView.render();return;} if(state.view==='map'){renderHome(true);$('#explore-section').classList.add('hidden');$('#map-section').classList.remove('hidden');$('#branch-section').classList.add('hidden');$('#landscape-section').classList.add('hidden');return;} if(state.view==='landscape'){renderLandscape();return;} renderHome(true); }
...
init();
window.__radarRerender = () => { syncFromUrl(); render(); };
